...
export class BackupsComponent extends FormTemplateComponent {
  ...
  async open(record: any) {
    ...
    if (record && record.node) {
      ...
      this._service.comList('operationprocesssnapshot', { other_key: this.node.pwb_key ? this.node.pwb_key : this.node.psopm_key }, 'getlist').then((v) => {
        this.operation = v;
      })
      ...
      if (!this._body.key) {
        this._body = Object.assign(this._body, record.power.com == true ? { pwb_key: this.node.pwb_key } : { psopm_key: this.node.psopm_key })
      }
