...
/**列表显示 */
@Component({
    selector: 'ina-list-show',
    templateUrl: './list-show.component.html',
    styleUrls: ['./list-show.component.less']
})
export class ListShowComponent implements OnInit, OnChanges {
    ...
    @Input() NegStatus = -1;
    /**进度计算公式参数 */
    @Input() statusnode = { all: 'quantity', current: 'complete_quantity' };
    /**是否统计 */
    ...
    /**调整列宽返回事件*/
    @Output() Resizewidth = new EventEmitter();
    /**操作栏宽度 */
    btnGroupW = 50;
    ngOnInit() {
        this.btnChange();
        this.tableCW = localStorage && localStorage.tableCW ? JSON.parse(localStorage.tableCW) : true;
        setTimeout(() => {
            if (!this.btnGroup || this.btnGroup == []) this.btnChange();
        }, 500);
        // window.addEventListener('tableCW', () => {
        //     console.log(111)
        // })
    }
    btnChange() {
        ...
        if (this.btnGroup.extend && this.btnGroup.extend.length > 0) this.btnGroupW = this.btnGroupW + 30;
        // console.log(this.btnGroup,this.btnGroupW)
    }
    ngOnChanges(change) {
    ...
    // getTipsMsg(condition, Dynamic?): string {
    //     const translate = AppConfig.translate.tipsMsg;
    ...
    onResize({ width }: NzResizeEvent, col: string): void {
        ...
        let _opw = this.isAction == true ? $('.Action').width() : 0;
        /**最大宽度 */
        let sw = this.columns.reduce((a, b) => {
            let aw = typeof a == 'number' ? a : (b.widthnum ? b.widthnum : parseInt(b.width.replace('px', '')));
            let bw = b.widthnum ? b.widthnum : parseInt(b.width.replace('px', ''));
            return aw + bw;
        }, 0)
        if (sw + _opw < $('.ant-table-header').width()) {
            let dvalue = 0;
            let currenti = 0;
            ...
            width = width < _W ? _W : width;
            this.columns = this.columns.map(e => (e.coums === col ? { ...e, width: `${width}px`, widthnum: width } : e));
        }
        if (this.isSetList == true) {
            let AllColumns = JSON.parse(localStorage.AllColumns);
            AllColumns[this.jsonname].forEach(a => {
                if (!a.widthnum) a.widthnum = parseInt(a.width.replace('px', ''))
                let item = this.columns.find(c => c.coums == a.coums)
                if (item) {
                    a.widthnum = parseInt(item.width.replace('px', ''))
                    a.width = item.width;
                }
            })
            this.columns = AllColumns[this.jsonname].filter(ac => ac.check == true);
            let setNode: any = {}
            setNode[this.jsonname] = this.columns;
            localStorage.setItem("setColumns2", JSON.stringify(setNode))
            localStorage.setItem("AllColumns", JSON.stringify(Object.assign({}, AllColumns)))
            this.Resizewidth.emit(AllColumns[this.jsonname])
        }
    }
    /**状态进度计算 */
    statusformula(data): number {
        switch (data[this.statusnode.all]) {
            case 0:
                return 0
            default:
                if (UtilService.isEmpty(data[this.statusnode.current])) { data[this.statusnode.current] = 0; }
                let n = data[this.statusnode.current] / data[this.statusnode.all];
