...
export class StyleFlowNew2Component extends FormTemplateComponent {
    constructor(private breakpointObserver: BreakpointObserver,) {
        super();
        this.modularInit("pdmStyle", 'pdm_style', false);
        this.url = 'pdm_style'
        let oplistnewbtn = AppConfig.btnGroup.single.find(b => b.action == "oplistnew");
    ...
    async open(record: any) {
        ...
        this.btnaction = record.btnaction ? record.btnaction : this.btnaction;
    ...
    btnEvent(event) {
        ...
        switch (event.action) {
            ...
            case 'Update':
                ...
                break;
            case 'setPW':
                let setbody = Object.assign({}, { title: 'PWRM', power: this.power, type: 'S', ischeck: true, other_node: this.node, node: { key: event.node.popm_key, psi_key: this.key } });
    ...
    /**数据获取 */
    getDate(url) {
        this.seniorModel = Object.assign({}, this.seniorModel, { psi_key: this.node.key })
        let body = Object.assign({}, this.pageMap, this.panels == 0 ? { psi_key: this.node.key } : this.seniorModel)
    ...
    onAction(body) {
        ...
        switch (body.action) {
            case 'add':
                body = Object.assign(body, { node: { psi_key: this.node.key } })
                ...
                super.save({ url: 'admin/OperationProcessMaster/Extend/Copy/', model: this.newmodel, doAction: 'post' }, (result) => {
                    if (result) {
                        ...
                        this._service.saveModel(this.otherUrl.StyleSave, 'post', pro, (scss) => {
                            body = Object.assign({}, { title: 'copynew', power: this.power, type: 'S', ischeck: true, other_node: this.node, node: { key: result, psi_key: this.node.pkey } })
                            this._Middlepage.open(body);
                        })
                        this.copyingcancel()
                        this.close()
