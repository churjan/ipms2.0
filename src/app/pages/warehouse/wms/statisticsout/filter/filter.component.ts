    ...
    submitForm(event? :KeyboardEvent) {
        if(!event || event.key == "Enter"){  
            ...
            if(queryParams.date?.length > 0){
                queryParams.beginoutdate = moment(queryParams.date[0]).format("YYYY-MM-DD")
                queryParams.endoutdate = moment(queryParams.date[1]).format("YYYY-MM-DD")
            }else{
                queryParams.beginoutdate = null
                queryParams.endoutdate = null
            }
